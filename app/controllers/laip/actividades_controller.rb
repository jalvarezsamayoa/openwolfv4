# encoding: utf-8
class Laip::ActividadesController < ApplicationController
  respond_to :html, :js
  ...
  def show
    @actividad = @solicitud.actividades.find(params[:id])
  ...
  def actualizar_usuarios
    ...
    if institucion.id == current_usuario.institucion_id
      @usuarios = institucion.usuarios.activos.enlaces.order("usuarios.nombre")
    else
      @usuarios = institucion.usuarios.activos.supervisores.order("usuarios.nombre")
    end
    respond_to do |format|
      format.js do
